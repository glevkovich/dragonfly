...
# ---------------------------------------------------------------------------
# Adaptive averaging (only active when averaging, i.e. effective runs > 1).
#
# Instead of a fixed run count, we run a ladder of full-matrix passes and keep going
# while any (version, mode, pipeline) cell is still noisier than MAX_RESULT_VARIANCE_PCT:
#   * MIN_RUNS              floor on runs whenever we average (always >= 3).
#   * ADAPTIVE_MAX_RUNS     ceiling of the ladder (the "boundary of repeats").
#   * RUN_STEP              how many extra passes to add each time we extend (3 -> 5 -> 7 ...).
# After each pass that reaches the current ceiling we check the accumulated samples; if every
# cell's full-set RPS spread is within threshold we stop early, otherwise we add RUN_STEP more
# passes (capped at ADAPTIVE_MAX_RUNS). The final report then trims EACH cell to its tightest
# >= MIN_RUNS subset, so a couple of noisy cells never drag the average of calm cells, and a cell
# that never settles falls back to averaging all of its samples (and is still flagged by the
# variance warning). Set ADAPTIVE_AVG=0 to restore the old fixed-run behavior.
# A single run (runs==1, e.g. the NUM_RUNS=1 smoke test) is never averaged and never extended.
# ---------------------------------------------------------------------------
MIN_RUNS="${MIN_RUNS:-3}"
ADAPTIVE_MAX_RUNS="${ADAPTIVE_MAX_RUNS:-7}"
RUN_STEP="${RUN_STEP:-2}"
...
# Adaptive averaging setup. Only engages when we are averaging (RUNS > 1). RUNS becomes the
# CURRENT ceiling and may grow up to _EFF_MAX_RUNS as the run loop extends it (see the ladder
# in the config block above). A single run stays a single run.
if ! [[ "$MIN_RUNS" =~ ^[0-9]+$ ]] || [[ "$MIN_RUNS" -lt 1 ]]; then
    echo "[!] Error: MIN_RUNS must be a positive integer. Got: '$MIN_RUNS'"; exit 1
fi
if ! [[ "$RUN_STEP" =~ ^[0-9]+$ ]] || [[ "$RUN_STEP" -lt 1 ]]; then
    echo "[!] Error: RUN_STEP must be a positive integer. Got: '$RUN_STEP'"; exit 1
fi
if ! [[ "$ADAPTIVE_MAX_RUNS" =~ ^[0-9]+$ ]] || [[ "$ADAPTIVE_MAX_RUNS" -lt 1 ]]; then
    echo "[!] Error: ADAPTIVE_MAX_RUNS must be a positive integer. Got: '$ADAPTIVE_MAX_RUNS'"; exit 1
fi
if [[ "$RUNS" -gt 1 && "$ADAPTIVE_AVG" == "1" ]]; then
    ADAPTIVE=1
    [[ "$RUNS" -lt "$MIN_RUNS" ]] && RUNS=$MIN_RUNS   # always average at least MIN_RUNS passes
    _EFF_MAX_RUNS=$ADAPTIVE_MAX_RUNS
    [[ "$_EFF_MAX_RUNS" -lt "$RUNS" ]] && _EFF_MAX_RUNS=$RUNS   # honor an explicit large request
else
    ADAPTIVE=0
    _EFF_MAX_RUNS=$RUNS
fi
...
# Adaptive-averaging convergence test. Echoes "1" if EVERY (version, mode, pipeline) cell has
# at least MIN_RUNS numeric samples AND its full-set RPS spread (max-min)/min*100 is within
# MAX_RESULT_VARIANCE_PCT; otherwise echoes "0". Used by the run loop to decide whether to add
# more passes. A cell that keeps erroring (rps<=0) never reaches MIN_RUNS samples, so it keeps
# the answer "0" and the ladder runs out to ADAPTIVE_MAX_RUNS (then stops).
_cells_converged() {
    [[ -s "$ACCUM_FILE" ]] || { echo 0; return; }
    awk -F'|' -v thr="$MAX_RESULT_VARIANCE_PCT" -v minn="$MIN_RUNS" '
    {
        key = $1 SUBSEP $2 SUBSEP $3
        rps = $4 + 0
        if (rps <= 0) next
        if (!(key in seen)) { seen[key] = 1; order[++n] = key; mn[key] = rps; mx[key] = rps }
        else { if (rps < mn[key]) mn[key] = rps; if (rps > mx[key]) mx[key] = rps }
        cnt[key]++
    }
    END {
        ...
        for (i = 1; i <= n; i++) {
            ...
            if (spread > thr) {
                if (!warned) {
                    print ""
                    print "======================================================"
                    printf "  [!] HIGH VARIANCE WARNING (threshold %.1f%%) - full-set spread; the\n", thr
                    print  "      AVERAGE RESULTS report trims each cell to its tightest >=MIN_RUNS subset"
                    print "======================================================"
    ...
    multi_conn|single_conn|pubsub|conn|all)
        {
            ...
            while (( r < RUNS )); do
                ...
                # Adaptive ladder: once we reach the current ceiling, decide whether to extend.
                # Extend by RUN_STEP (capped at _EFF_MAX_RUNS) while any cell's full-set RPS spread
                # is still above MAX_RESULT_VARIANCE_PCT. We add whole matrix passes (not per-cell
                # reruns); the final report trims each cell to its tightest >= MIN_RUNS subset, so
                # calm cells are unaffected by the extra passes a noisy cell triggers.
                if (( ADAPTIVE )) && (( r == RUNS )) && (( r >= MIN_RUNS )) && (( RUNS < _EFF_MAX_RUNS )); then
                    if [[ "$(_cells_converged)" == "0" ]]; then
                        _new=$(( RUNS + RUN_STEP ))
                        (( _new > _EFF_MAX_RUNS )) && _new=$_EFF_MAX_RUNS
                        echo ""
                        echo "############## [adaptive] >${MAX_RESULT_VARIANCE_PCT}% spread after ${r} run(s) -> extending ${RUNS} -> ${_new} ##############"
                        RUNS=$_new
                    else
                        echo ""
                        echo "############## [adaptive] all cells within ${MAX_RESULT_VARIANCE_PCT}% after ${r} run(s) -> stopping ##############"
                    fi
                fi
            done
            print_final_report
