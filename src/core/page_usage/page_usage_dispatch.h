...
//
// LAYERING: this header includes server/defrag.h to see the concrete
// subclass definitions (Evacuator, CensusTaker), and mimalloc/internal.h
// for the `_mi_ptr_page` used inside Evacuator::*Impl. mimalloc/internal.h
// is NOT pulled into defrag.h — including it there leaks into
// engine_shard.h → main_service.h → dfly_main.cc, where it conflicts with
// dfly_main.cc's `extern "C"` redeclarations. Keeping it scoped to this
// header (and the TUs that include it) avoids that.
//
