...
# alone does NOT catch it. -fsanitize=implicit-conversion (enabled by WITH_USAN
# on Clang) is required.
#
# How to run:
...
#
# Expected result on the BUGGY branch (ubsan-poc-7562-revert) + UBSan binary:
#   TEST PASSES and prints to terminal:
...
#
# Expected result on the FIXED branch (ubsan-detection-improvements) + UBSan binary:
    ...
    async def test_ubsan_detects_implicit_conversion(self, df_factory):
        ...
        if not ubsan_files:
            pytest.skip(
                ...
                "-fsanitize=implicit-conversion (WITH_USAN=ON + Clang), "
        ...
        src_findings = [
            ...
            if "/src/" in line and "runtime error" in line
        ]
        if src_findings:
