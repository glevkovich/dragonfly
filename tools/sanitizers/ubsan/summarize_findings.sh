#!/usr/bin/env bash
#
# Summarize UBSan findings from the connection_test.py run into GitHub-flavored
# markdown, written to stdout. The weekly workflow appends it to the job summary:
#
#   bash summarize_findings.sh <ubsan-logs-dir> <arch> >> "$GITHUB_STEP_SUMMARY"
#
# Why a script (not inline YAML): the classification + formatting is long and
# fiddly; keeping it here makes ubsan-weekly.yml short and lets us test locally.
#
# Color in job summaries: GitHub strips inline CSS, so we colorize via
#   - GitHub "alerts" (blockquote callouts): [!CAUTION] = red, [!NOTE] = blue.
#   - ```diff fences: lines beginning with '+' render green.
#
set -euo pipefail

logs_dir="${1:?usage: summarize_findings.sh <logs-dir> <arch>}"
arch="${2:?usage: summarize_findings.sh <logs-dir> <arch>}"
...
emit_section() {
  ...
  if [[ "${bucket}" == "UB" ]]; then
    echo "> [!CAUTION]"
    echo "> **Undefined behavior — ${total_n} occurrence(s).** These are **real C++ undefined behavior**: code that violates the C++ standard (see ${UB_LINK})."
  else
    echo "> [!NOTE]"
    echo "> **Suspicious / defined-but-flagged — ${total_n} occurrence(s).** Well-defined behavior surfaced by the extra integer & implicit-conversion checks (unsigned wrap/shift/negation, narrowing). Not standard violations; review for intent."
  fi
  echo ""
