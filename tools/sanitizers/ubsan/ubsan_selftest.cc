...
// The PR #7562 pattern: 1ULL<<32 narrowed to unsigned silently becomes 0.
...
// NOTE: object-size is the one check that genuinely needs the optimizer -- it is
// computed from __builtin_object_size, which yields "unknown" under -O0/optnone.
// So this case must NOT use UBSAN_CASE (which adds optnone); it keeps the build's
// -O1 (noinline still gives a clean standalone frame). It also stores through a
// pointer rather than calling memset: object-size instruments scalar loads/stores,
// not the memset libcall.
__attribute__((noinline)) void Case_ObjectSize() {
  char buf[4] = {};
  char* p = buf;  // pointer, not array: array-bounds won't instrument p[]; object-size will.
  // The offset MUST be a compile-time constant: object-size emits
  //   if (llvm.objectsize(p + off) < needed) handle();
  // and objectsize(p + off) only folds to a known value (here 0, since off is past
  // the 4-byte object) for a constant off. A runtime/volatile index folds to
  // "unknown" (SIZE_MAX) and the check is never taken. The handler is a call with
  // side effects, so it survives even though the store itself is dead.
