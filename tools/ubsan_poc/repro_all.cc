...
__attribute__((noinline)) unsigned narrow_unsigned(unsigned v) { return v; }
...
__attribute__((noinline)) int16_t narrow_signed(int64_t v) { return v; }
...
__attribute__((noinline)) int sadd(int a, int b) { return a + b; }
...
__attribute__((noinline)) unsigned uadd(unsigned a, unsigned b) { return a + b; }
...
__attribute__((noinline)) int8_t inc_char(int8_t c) { return c + 1; }
...
int main(int argc, char** argv) {
  ...
  if (which == 0 || which == 1) case1_unsigned_truncation();
  if (which == 0 || which == 2) case2_signed_truncation();
  if (which == 0 || which == 3) case3_sign_change();
  if (which == 0 || which == 4) case4_signed_overflow();
  if (which == 0 || which == 5) case5_unsigned_overflow();
  if (which == 0 || which == 6) case6_char_overflow();
  return 0;
}
